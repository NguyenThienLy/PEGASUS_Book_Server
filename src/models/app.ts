...
const AppSchema = sequelize.define("tbl_app",
    {
        ...
        ownerId: { type: Sequelize.UUID, references: { model: 'tbl_user', key: '_id' } }
    },
    {
        timestamps: true,
...
AppSchema.associate = (models: any) => {

    AppSchema.belongsTo(models['User'], {
        foreignKey: 'ownerId',
        as: 'user'
    });
    models['User'].hasMany(AppSchema, {
        foreignKey: 'ownerId',
        as: 'apps'
    });
};
